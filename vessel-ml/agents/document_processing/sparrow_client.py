...
class VesselClient:
    ...
    @task(name="extract_table_vessel", retries=2)
    async def extract_table(self, page: Any) -> Dict:
        """
        Sends request to extract tabular data using Vessel API
        """
        if self.mock_mode:
    ...
    @task(name="extract_form_vessel", retries=2)
    async def extract_form(self, page: Any) -> Dict:
        """
        Sends request to extract form data using Vessel API
        """
        if self.mock_mode:
