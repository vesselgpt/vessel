...
class VesselClient:
    """
    Client for interacting with Vessel API endpoints.
    """
    ...
    @task(name="extract_type_per_page_vessel", retries=2)
    async def extract_type_per_page_vessel(self, doc: dict) -> Dict:
        """
        Sends request to extract document pages using Vessel API
        ...
        endpoint = urljoin(self.base_url, "/api/v1/vessel-llm/inference")
        ...
        try:
            ...
            form_data.add_field('pipeline', 'vessel-parse')
            ...
            form_data.add_field('vessel_key', '')
    ...
    @task(name="extract_data_vessel", retries=2)
    async def extract_data_vessel(self, content: bytes, params: Dict[str, Any]) -> Dict:
        """
        Sends request to extract data from a specific page using Vessel API
