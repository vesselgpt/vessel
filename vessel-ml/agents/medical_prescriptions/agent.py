...
async def detect_doc_structure(input_data: Dict, vessel_client: VesselClient) -> Dict:
    """
    Extracts each page type from a document using Vessel API
    """

    if 'content' not in input_data:
        raise ValueError("Document data is required")

    results = await vessel_client.extract_type_per_page_vessel(input_data)
